...
public class StudentController {
    ...
    public ResponseEntity<Student> getAttendanceById(@PathVariable int id) {
    ...
    public ResponseEntity<List<Student>> getAllAttendances() {
        return new ResponseEntity<>(studentService.getAllStudents(), HttpStatus.OK);
    }

    @ApiResponse(responseCode = "200", description = "Successful retrieval of students in the same Department", content = @Content(array = @ArraySchema(schema = @Schema(implementation = Student.class))))
    @Operation(summary = "Get students by Department", description = "Retrieves all the students whoever in the Department.")
    @GetMapping(value = "/{department}/all", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<Student>> getAttendanceById(@PathVariable String department) {
        return new ResponseEntity<>(studentService.getAllStudentsByDepartment(department), HttpStatus.OK);
    }
